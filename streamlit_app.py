...
def model_feature_names(model):
    """Return the exact feature order expected by the fitted estimator."""
...
def aligned_input(model, overrides=None):
    """
    Build an inference DataFrame using the exact columns/order stored by
    CatBoost/LightGBM. Missing columns are safely filled with 0.
    """
...
# ========================= POWER =============================
    ...
    st.caption("Inference is automatically aligned to the exact feature schema used during training.")

    alg = st.radio("Inference engine", ["CatBoost", "LightGBM"], horizontal=True)
    model = models.get(f"Power / {alg}")

    if not model:
        st.warning("Power model is not loaded. Make sure the 8 model artifacts exist in models/.")
    else:
        expected = model_feature_names(model)
        st.success(f"Model ready • {len(expected)} features detected")
        ...
        if st.button("RUN POWER FORECAST", type="primary", use_container_width=True):
            try:
                ...
                with c2:
                    max_power = (
                        max(float(df["active_power"].max()) * 1.1, 1)
                        if "active_power" in df else max(pred * 1.5, 1)
                    )
                    fig = go.Figure(go.Indicator(
                        mode="gauge+number",
                        value=max(pred, 0),
                        title={"text": "Predicted Power"},
                        gauge={"axis": {"range": [0, max_power]}}
                    ))
                    chart(fig, 300)
            except Exception as e:
                st.error(f"Power inference failed: {e}")

# ========================= FAULT =============================
elif page == "Fault Intelligence":
    st.title("◉ Fault Intelligence")
    st.caption("Binary health classification with automatic feature alignment.")

    alg = st.radio("Classifier", ["CatBoost", "LightGBM"], horizontal=True)
    model = models.get(f"Fault / {alg}")

    if not model:
        st.warning("Fault model is not loaded.")
    else:
        expected = model_feature_names(model)
        vals = {}
        cols = st.columns(3)
        ...
        if st.button("ANALYZE SYSTEM HEALTH", type="primary", use_container_width=True):
            try:
                ...
                with c1:
                    kpi(
                        "SYSTEM STATUS",
                        "FAULT" if pred else "HEALTHY",
                        f"{prob:.1%} fault probability"
                    )
                with c2:
                    fig = go.Figure(go.Indicator(
                        mode="gauge+number",
                ...
                st.error(f"Fault inference failed: {e}")

# ========================= ROOT CAUSE ========================
elif page == "Root Cause":
    st.title("⌁ Root Cause Intelligence")
    st.caption("Historical fault mix plus live Model 3 diagnosis.")
    ...
    alg = st.radio("Live diagnosis engine", ["CatBoost", "LightGBM"], horizontal=True, key="cause_alg")
    model = models.get(f"Cause / {alg}")

    if model and st.button("RUN ROOT-CAUSE DIAGNOSIS", type="primary", use_container_width=True):
        try:
            X = aligned_input(model, {})
            pred = np.asarray(model.predict(X)).ravel()[0]
            st.success(f"Predicted cause: {pred}")
            ...
            st.error(f"Root-cause inference failed: {e}")

# ========================= SEVERITY ==========================
elif page == "Severity Monitor":
    st.title("▣ Severity Monitor")
    st.caption("Operational view of historical and predicted fault severity.")
    ...
    if "fault_severity" in df:
        ...
        for col, label in zip(c, counts.index):
            with col:
                kpi(label, f"{int(counts[label]):,}", "observations")
    ...
    alg = st.radio("Severity engine", ["CatBoost", "LightGBM"], horizontal=True, key="sev_alg")
    model = models.get(f"Severity / {alg}")

    if model and st.button("ASSESS CURRENT SEVERITY", type="primary", use_container_width=True):
        try:
            X = aligned_input(model, {})
            pred = np.asarray(model.predict(X)).ravel()[0]
            labels = {0: "None", 1: "Low", 2: "Medium", 3: "High"}
            st.success(f"Predicted severity: {labels.get(int(pred), str(pred))}")
        except Exception as e:
            st.error(f"Severity inference failed: {e}")
